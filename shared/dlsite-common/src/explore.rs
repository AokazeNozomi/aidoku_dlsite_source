...
/// Parameters for explore catalog search requests.
pub struct ExploreSearchParams<'a> {
	pub site_slug: &'a str,
	pub keyword: Option<&'a str>,
	pub page: i32,
	pub sort: ExploreSort,
	pub languages: &'a [DlsiteLang],
	pub work_types: &'a [String],
	pub content_ratings: &'a [String],
	pub genres: &'a [u32],
}

impl<'a> ExploreSearchParams<'a> {
	pub fn new(site_slug: &'a str, page: i32, sort: ExploreSort) -> Self {
		Self {
			site_slug,
			keyword: None,
			page,
...
pub fn build_search_url(params: &ExploreSearchParams<'_>) -> String {
	let base = format!("https://www.dlsite.com/{}", params.site_slug);
	...
	for cr in params.content_ratings {
	...
	for (i, wt) in params.work_types.iter().enumerate() {
		path.push_str(&format!("/work_type%5B{}%5D/{}", i, wt));
	}

	// Genres — use indexed array params
	for (i, gid) in params.genres.iter().enumerate() {
	...
	// Sort order
	path.push_str(&format!("/order%5B0%5D/{}", params.sort.order_param()));

	// Page
	path.push_str(&format!("/page/{}", params.page));
...
pub fn search_explore(params: &ExploreSearchParams<'_>) -> Result<ExploreResult> {
	...
	let result = parse_fsr_ajax_response(&data, params.page)?;
