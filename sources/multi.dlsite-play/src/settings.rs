...
pub use dlsite_common::settings::{
	clear_web_cookies, get_web_cookies, is_logged_in, set_logged_in, set_web_cookies,
};
...
pub fn set_cached_genres(value: &str) {
	defaults_set(CACHED_GENRES_KEY, DefaultValue::String(String::from(value)));
	defaults_set(
...
pub fn set_cached_series_map(title_id: &str, worknos: &[String]) {
	let joined: String = worknos.join(",");
	defaults_set(&series_map_key(title_id), DefaultValue::String(joined));
}
...
pub fn set_last_viewed_workno(workno: &str) {
	defaults_set(LAST_VIEWED_KEY, DefaultValue::String(String::from(workno)));
}
