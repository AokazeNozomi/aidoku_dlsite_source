use super::http::{
	PLAY_API, PLAY_BASE, PLAY_DL_API, body_preview, ensure_ok, play_authenticated_get,
	play_post_json,
};
...
/// Returns the combined `WorksResponse`.
pub fn get_works(worknos: &[String]) -> Result<WorksResponse> {
	get_works_chunked(worknos, |_, _| {})
}

/// Like [`get_works`], but invokes `on_chunk` after each chunk is fetched and
/// parsed. The callback receives only the freshly parsed chunk (not the
/// accumulation), so chunk 0 corresponds to the first 100 work IDs.
pub fn get_works_chunked<F: FnMut(usize, &WorksResponse)>(
	worknos: &[String],
	mut on_chunk: F,
) -> Result<WorksResponse> {
