...
use super::models::{OptimizedInfo, PlayFile, PlayFileFiles, ZipTree};
...
/// Validate that a downloaded crypt image is complete before descrambling.
pub fn validate_crypt_image(
	response: &aidoku::ImageResponse,
	ctx: &aidoku::PageContext,
	width: i32,
	height: i32,
) -> Result<()> {
	let opt_name = ctx.get("optimized_name").map(|s| s.as_str()).unwrap_or("");

	if !(200..300).contains(&(response.code as i32)) {
		debug_print!(
			"[dlsite-play] crypt image HTTP {} for {} (expected {}x{})",
			response.code,
			opt_name,
			width,
			height
		...
		bail!(
			"Crypt image download failed with HTTP {} for {}",
			response.code,
			opt_name
		);
	}

	let img_w = response.image.width() as i32;
	let img_h = response.image.height() as i32;
	if img_w == 0 || img_h == 0 {
		debug_print!(
			"[dlsite-play] crypt image has zero dimensions for {} ({}x{})",
			opt_name,
			img_w,
			img_h
		);
		bail!("Crypt image has zero dimensions for {}", opt_name);
	}
	// Crypt images are split into 128px tiles, so the encoded image is padded
	// out to the next whole multiple of the tile size in each dimension.
	const TILE: i32 = 128;
	let pad_w = ((width + TILE - 1) / TILE) * TILE;
	let pad_h = ((height + TILE - 1) / TILE) * TILE;
	if img_w != pad_w || img_h != pad_h {
		debug_print!(
			"[dlsite-play] crypt image dimension mismatch for {}: got {}x{}, expected {}x{} (padded from {}x{})",
			opt_name,
			img_w,
			img_h,
			pad_w,
		...
		bail!(
			"Crypt image dimension mismatch for {}: got {}x{}, expected {}x{}",
			opt_name,
			img_w,
			img_h,
			pad_w,
	...
	if let Some(expected_len) = ctx.get("length").and_then(|s| s.parse::<i64>().ok()) {
		let content_length = response
			.headers
			.iter()
			.find(|(k, _)| k.eq_ignore_ascii_case("content-length"))
			.and_then(|(_, v)| v.parse::<i64>().ok());
		if let Some(actual_len) = content_length
			&& actual_len < expected_len
		{
			debug_print!(
				"[dlsite-play] crypt image Content-Length too small for {}: got {}, expected {}",
				opt_name,
				actual_len,
				expected_len
			);
			bail!(
				"Crypt image incomplete for {}: Content-Length {} < expected {}",
				opt_name,
				actual_len,
				expected_len
			);
...
/// Returns true when the CDN token or session likely expired.
pub fn is_auth_failure(code: u16) -> bool {
	code == 401 || code == 403
}

/// Returns true if another fetch attempt is allowed.
pub fn should_retry(attempt: u32, max_attempts: u32) -> bool {
	attempt + 1 < max_attempts
}

/// Fetch a fresh optimized image URL after CDN token expiry.
pub fn refresh_optimized_url(ctx: &aidoku::PageContext) -> Result<String> {
	let workno = ctx
		.get("workno")
		.filter(|s| !s.is_empty())
		.ok_or_else(|| error!("Missing workno for crypt page URL refresh"))?;
	let opt_name = ctx
		.get("optimized_name")
		.filter(|s| !s.is_empty())
		.ok_or_else(|| error!("Missing optimized_name for crypt page URL refresh"))?;
	let token = super::api::download_token(workno)?;
	Ok(super::api::optimized_url(&token, opt_name))
}

/// Download a crypt page image and wrap it as an [ImageResponse] for validation.
pub fn fetch_crypt_image(url: &str) -> Result<aidoku::ImageResponse> {
	use aidoku::{HashMap, ImageRequest, ImageResponse};

	let resp = super::http::play_image_get(url)?
		.send()
		.map_err(|_| error!("Crypt image fetch failed for {}", url))?;
	let status = resp.status_code();
	if status < 0 {
		bail!("Crypt image fetch failed for {}", url);
	}
	let image = resp
		.get_image()
		.map_err(|_| error!("Crypt image decode failed for {}", url))?;
	let mut headers = HashMap::new();
	if let Some(cl) = resp.get_header("Content-Length") {
		headers.insert("Content-Length".into(), cl);
	}
	Ok(ImageResponse {
		code: status as u16,
		headers,
...
fn try_process_crypt_response(
	...
	validate_crypt_image(response, ctx, width, height)?;
	descramble_image(&response.image, opt_name, width, height)
}

/// Validate, descramble, and retry crypt page images for reliable offline downloads.
pub fn process_crypt_image(
	initial: aidoku::ImageResponse,
	ctx: &aidoku::PageContext,
	width: i32,
	height: i32,
) -> Result<ImageRef> {
	...
	let mut url = initial
		...
		.url
		.clone()
		.ok_or_else(|| error!("Missing image URL for crypt page {}", opt_name))?;
	let mut response = initial;
	let mut last_err = error!("Crypt image processing failed for {}", opt_name);

	for attempt in 0..MAX_ATTEMPTS {
		match try_process_crypt_response(&response, ctx, width, height, &opt_name) {
			Ok(img) => return Ok(img),
			Err(e) => {
				debug_print!(
					"[dlsite-play] crypt image attempt {} failed for {} HTTP {}: {:?}",
					attempt + 1,
					opt_name,
					response.code,
					e
				...
				if is_auth_failure(response.code) {
					url = refresh_optimized_url(ctx)?;
				}
				response = match fetch_crypt_image(&url) {
					Ok(r) => r,
					Err(e) => {
						debug_print!(
							"[dlsite-play] crypt image refetch failed for {} attempt {}: {:?}",
							opt_name,
							attempt + 2,
							e
						);
...
pub fn descramble_image(
	...
	// The scrambled image is padded out to whole 128px tiles, so its decoded
	// dimensions are the logical size rounded up to the next tile boundary.
	let pad_w = tiles_w * TILE_W;
	let pad_h = tiles_h * TILE_W;
	let img_w = image.width();
	let img_h = image.height();
	if img_w as i32 != pad_w || img_h as i32 != pad_h {
		bail!(
...
mod tests {
	...
	fn test_crypt_context() -> aidoku::PageContext {
		let mut ctx = aidoku::PageContext::new();
		ctx.insert("optimized_name".into(), "abcde1234567.webp".into());
		ctx.insert("length".into(), "5000".into());
		ctx
	}
	...
	fn test_image_response(
		...
		image: ImageRef,
		headers: &[(&str, &str)],
	) -> aidoku::ImageResponse {
		let mut hdrs = aidoku::HashMap::new();
		for (k, v) in headers {
	...
	#[aidoku_test]
	fn validate_crypt_image_rejects_http_error() {
		let image = ImageRef::new(PNG_1X1);
		let response = test_image_response(403, image, &[]);
		let ctx = test_crypt_context();
		assert!(validate_crypt_image(&response, &ctx, 1, 1).is_err());
	}

	#[aidoku_test]
	fn validate_crypt_image_rejects_dimension_mismatch() {
		let image = ImageRef::new(PNG_1X1);
		let response = test_image_response(200, image, &[]);
		let ctx = test_crypt_context();
		assert!(validate_crypt_image(&response, &ctx, 800, 600).is_err());
	}

	#[aidoku_test]
	fn validate_crypt_image_rejects_short_content_length() {
		let image = ImageRef::new(PNG_1X1);
		let response = test_image_response(200, image, &[("Content-Length", "100")]);
		let ctx = test_crypt_context();
		assert!(validate_crypt_image(&response, &ctx, 1, 1).is_err());
	}

	#[aidoku_test]
	fn descramble_image_rejects_dimension_mismatch() {
		let image = ImageRef::new(PNG_1X1);
		assert!(descramble_image(&image, "abcde1234567.webp", 128, 128).is_err());
	}

	#[aidoku_test]
	fn is_auth_failure_detects_401_and_403() {
	...
	#[aidoku_test]
	fn refresh_optimized_url_requires_workno() {
		let mut ctx = aidoku::PageContext::new();
		ctx.insert("optimized_name".into(), "abcde1234567.webp".into());
		assert!(refresh_optimized_url(&ctx).is_err());
	}

	#[aidoku_test]
	fn refresh_optimized_url_requires_optimized_name() {
		let mut ctx = aidoku::PageContext::new();
		ctx.insert("workno".into(), "RJ123456".into());
		assert!(refresh_optimized_url(&ctx).is_err());
	}
	...
	fn parent_folder_of_nested_path() {
		assert_eq!(
			parent_folder_path("folder/sub/file.jpg"),
