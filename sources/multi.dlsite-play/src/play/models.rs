...
impl PlayFile {
	...
	pub fn optimized_length(&self) -> Option<i64> {
		self.files.optimized.as_ref()?.length
	}
}
...
mod tests {
	...
	#[aidoku_test]
	fn playfile_optimized_length_present() {
		let pf = make_playfile(Some("x.webp"), Some(true), Some(800), Some(600));
		assert_eq!(pf.optimized_length(), Some(50));
	}

	#[aidoku_test]
	fn playfile_optimized_length_missing() {
		let pf = PlayFile {
			length: 100,
			file_type: "image".into(),
			files: PlayFileFiles {
		...
		assert_eq!(pf.optimized_length(), None);
	}
}
