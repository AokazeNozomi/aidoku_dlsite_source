...
pub(super) fn ensure_ok(op: &str, status: i32, data: &[u8]) -> Result<()> {
	...
	if status == 401 {
		bail!(
			"{} HTTP 401: session expired, complete web login again.",
			op
		);
	}
...
mod tests {
	...
	fn percent_decode_multiple_encoded() {
		assert_eq!(percent_decode_cookie_value("a%20b%20c"), "a b c");
	}
	...
	fn accept_for_api_v3_url() {
		assert_eq!(
			accept_for_url("https://play.dlsite.com/api/v3/content/sales"),
			"application/json"
		);
	}
	...
	fn accept_for_api_url() {
		assert_eq!(
			accept_for_url("https://play.dlsite.com/api/authorize"),
			"application/json"
		);
	}
	...
	fn accept_for_non_api_url() {
		assert_eq!(
			accept_for_url("https://play.dlsite.com/work/RJ123/view"),
			"*/*"
		);
	}
}
