...
impl PurchaseWork {
	...
	pub fn into_manga_with_date(
		...
		if !translated_by.is_empty() {
			...
			&& let Some(ref name) = translator.name {
				desc_lines.push(format!("Translator: {}", name.best()));
			}
		// Series info
		if let Some(ref ws) = self.series
			&& let Some(series_name) = series_names.get(&ws.title_id) {
...
fn is_degenerate_name(name: &str) -> bool {
	const STOP_WORDS: &[&str] = &[
		"a", "an", "the", "in", "on", "of", "to", "at", "by", "or", "and", "for", "but", "not",
		"its", "my", "is", "it", "this", "that", "with", "from", "some", "than",
	];
...
mod tests {
	...
	fn derive_series_name_accepts_long_prefix() {
		...
		assert_eq!(derive_series_name(&works), Some("Dragon Quest".to_string()));
	}
	...
	fn purchase_work_cover_url_protocol_relative() {
		...
		assert_eq!(
			w.cover_url(),
			Some("https://img.dlsite.jp/work/123.jpg".to_string())
		);
	}
