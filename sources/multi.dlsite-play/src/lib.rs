...
use aidoku::{
	...
	HomePartialResult, ImageRequestProvider, ImageResponse, Link, Listing, ListingKind,
	ListingProvider, Manga, MangaPageResult, NotificationHandler, Page, PageContent,
	PageContext, PageImageProcessor, Result, Source, WebLoginHandler,
	alloc::{
		String, Vec,
...
struct WorkFilterParams<'a> {
	q_lower: Option<&'a str>,
	q_raw: Option<&'a str>,
	work_types: &'a [String],
	work_type_exclude: &'a [String],
	translation_filter: Option<&'a str>,
	genre_filter: &'a [u32],
	genre_exclude: &'a [u32],
	content_rating_filter: &'a [String],
	language_filter: &'a [String],
...
impl Source for DlsitePlay {
	...
	fn get_page_list(&self, manga: Manga, chapter: Chapter) -> Result<Vec<Page>> {
		...
		let is_first_chapter = chapter
			...
			.unwrap_or(false);
		if is_first_chapter
			&& let Some(cover_url) = work_cover_url(workno) {
				result.push(Page {
		...
		result.extend(pages.into_iter().map(|(_path, pf)| {
			...
			if pf.is_crypt() {
				...
				context.insert("workno".into(), workno.to_string());
				if let Some((w, h)) = pf.crypt_dimensions() {
					context.insert("width".into(), w.to_string());
					context.insert("height".into(), h.to_string());
				}
				if let Some(len) = pf.optimized_length() {
					context.insert("length".into(), len.to_string());
				}
			}
...
impl DlsitePlay {
	...
	fn update_single_manga(
		...
		if needs_details || needs_chapters {
			...
			let resp = play::get_works(core::slice::from_ref(&manga.key)).ok();
	...
	fn update_series_manga(
		...
		if member_worknos.is_empty() {
			debug_print!("[dlsite-play] series {} has no cached members", title_id);
			return Ok(());
		}
		...
		if needs_details {
			...
			let earliest_purchase: Option<String> = works
				...
				.min()
				.cloned();
			let updated = models::series_manga(
				&title_id,
				&sname,
				&works,
				&genre_names,
				earliest_purchase.as_deref(),
			);
			manga.copy_from(updated);
		...
		match listing.id.as_str() {
			"library" => {
				...
				get_manga_list_inner(
					...
					ListFilterParams {
						work_types,
						work_type_exclude: Vec::new(),
						translation_filter: None,
						genre_filter: Vec::new(),
						genre_exclude: Vec::new(),
						content_rating_filter,
						language_filter,
					},
					SortParams {
...
impl Home for DlsitePlay {
	fn get_home(&self) -> Result<HomeLayout> {
		...
		// Render placeholder skeletons immediately so the home isn't blank
		// while the (potentially multi-request) works fetch runs.
		send_partial_result(&HomePartialResult::Layout(HomeLayout {
			components: Vec::from([
				HomeComponent {
					title: Some(String::from("Recently Purchased")),
					subtitle: None,
					value: HomeComponentValue::empty_scroller(),
				},
				HomeComponent {
		...
		let resp = play::get_works_chunked(&worknos, |chunk_idx, chunk| {
			if chunk_idx != 0 {
				return;
			}
			let work_refs: Vec<&models::PurchaseWork> = chunk.works.iter().collect();
			let genre_names = resolve_genre_names(&work_refs);
			let series_names = build_series_lookup(&chunk.series);
			let preview_entries = build_recently_purchased_links(
				&chunk.works,
				&work_types,
				&content_rating_filter,
				&lang_filter,
		...
		// --- Recently Purchased carousel (authoritative, full library) ---
		let purchased_entries = build_recently_purchased_links(
			&resp.works,
			&work_types,
			&content_rating_filter,
			&lang_filter,
		...
		let purchased_component = if purchased_entries.is_empty() {
			None
		} else {
			Some(HomeComponent {
				title: Some(String::from("Recently Purchased")),
		...
		// Individual works sorted by regist_date descending, deduplicated by
		// series title_id, top 20.
		let mut recent: Vec<&models::PurchaseWork> = resp
			.works
			.iter()
			.filter(|w| {
		...
		// Stream the Recently Released carousel before the view-history request.
		let released_component = if carousel_entries.is_empty() {
			None
		} else {
		...
		// Stream the Recently Read carousel once the view-history request lands.
		let read_component = if read_entries.is_empty() {
			None
		} else {
			let component = HomeComponent {
				title: Some(String::from("Recently Read")),
		...
		// Stream the Library preview last.
		let library_component = if library_links.is_empty() {
			None
		} else {
			let component = HomeComponent {
				title: Some(String::from("Library")),
		...
		// Assemble the final layout in display order, dropping empty sections
		// (this also clears any placeholders that produced no results).
		let mut components = Vec::new();
		components.extend(purchased_component);
		components.extend(released_component);
		components.extend(read_component);
		components.extend(library_component);
...
impl WebLoginHandler for DlsitePlay {
	fn handle_web_login(&self, key: String, cookies: HashMap<String, String>) -> Result<bool> {
		if key != "login" {
			debug_print!("[dlsite-play] web login rejected invalid key `{key}`");
...
impl PageImageProcessor for DlsitePlay {
	fn process_page_image(
		...
		let width = ctx
			...
			.filter(|&w| w > 0)
			.ok_or_else(|| error!("Missing width metadata for crypt page {}", opt_name))?;
		let height = ctx
			.get("height")
			.and_then(|s| s.parse::<i32>().ok())
			.filter(|&h| h > 0)
			.ok_or_else(|| error!("Missing height metadata for crypt page {}", opt_name))?;

		play::process_crypt_image(response, &ctx, width, height)
	}
}
...
/// Build the "Recently Purchased" carousel links: filter by settings, sort by
/// sales date descending, deduplicate by series title_id, top 20.
fn build_recently_purchased_links(
	...
	let mut purchased: Vec<&models::PurchaseWork> = works
		.iter()
		.filter(|w| {
			if !work_types.is_empty()
				&& !w
	...
	let mut seen_purchased_series: Vec<String> = Vec::new();
	...
	for w in &purchased {
		...
		if let Some(ref ws) = w.series {
			if seen_purchased_series.contains(&ws.title_id) {
				continue;
			}
			seen_purchased_series.push(ws.title_id.clone());
		}
		entries.push((*w).clone().into_manga(genre_names, series_names).into());
	}
	entries
}
...
fn work_passes_filter(w: &models::PurchaseWork, filter: &WorkFilterParams<'_>) -> bool {
	if let Some(q) = filter.q_lower {
		let name = w
			.name
			.as_ref()
			.map(|n| n.best())
		...
		let maker = w
			...
			.unwrap_or_default()
			.to_lowercase();
		let raw = filter.q_raw.unwrap_or_default();
		let sname = filter
			.series_name
			.map(|s| s.to_lowercase())
			.unwrap_or_default();
		if !(name.contains(q) || maker.contains(q) || w.workno.contains(raw) || sname.contains(q)) {
			return false;
		}
	}
	if !filter.work_types.is_empty() {
		let wt = w.work_type.as_deref().unwrap_or("");
		if !filter.work_types.iter().any(|t| t == wt) {
			return false;
		}
	}
	if !filter.work_type_exclude.is_empty() {
		let wt = w.work_type.as_deref().unwrap_or("");
		if filter.work_type_exclude.iter().any(|t| t == wt) {
			return false;
		}
	}
	if let Some(tf) = filter.translation_filter {
	...
	if !filter.genre_filter.is_empty() {
		let has_genre = filter
			.genre_filter
			.iter()
	...
	if !filter.genre_exclude.is_empty()
		&& filter
			.genre_exclude
			.iter()
			.any(|gid| w.genre_ids.contains(gid))
	{
		return false;
	}
	if !w.matches_content_ratings(filter.content_rating_filter) {
		return false;
	}
	if !settings::work_matches_languages(&w.workno, filter.language_filter) {
		return false;
	}
	true
}
...
fn build_sorted_entries(
	...
	let base_filter = WorkFilterParams {
		q_lower: q_lower.as_deref(),
		q_raw: query.as_deref(),
		work_types: &work_types,
		work_type_exclude: &work_type_exclude,
		translation_filter: translation_filter.as_deref(),
	...
	for key in &group_order {
		if let Some(works) = series_groups.get(key.as_str()) {
			...
			if has_filter {
				let series_filter = WorkFilterParams {
					series_name: sname,
					..base_filter
				};
				let any_match = works
					.iter()
					.any(|w| work_passes_filter(w, &series_filter));
				if !any_match {
					continue;
				}
			}
			...
			all_entries.push((
				sort_key,
				models::series_manga(key, name, works, &genre_names, earliest_purchase.as_deref()),
			));
		} else if let Some(pos) = standalone.iter().position(|(k, _)| k == key) {
			let (_, w) = &standalone[pos];
			if has_filter && !work_passes_filter(w, &base_filter) {
				continue;
			}
			...
			all_entries.push((
				sort_key,
				w.into_manga_with_date(&genre_names, &series_names, pd),
			));
		}
	}
...
fn get_manga_list_inner(
	...
	let all_entries = build_sorted_entries(&sales_dates, resp, query, filters, sort);
...
fn apply_sort(entries: &mut [(SortKey, Manga)], sort_option: SortOption, ascending: bool) {
...
fn get_or_fetch_worknos(page: i32) -> Result<(Vec<String>, BTreeMap<String, String>)> {
	...
	if !settings::is_logged_in() {
		debug_print!("[dlsite-play] get_or_fetch_worknos skip: not logged in (Account → Login)");
	...
	if page == 1 {
		...
		debug_print!("[dlsite-play] refreshed sales list count={}", worknos.len());
		...
		if cached.is_empty() {
			...
			let cached_dates = settings::get_cached_sales_dates();
			debug_print!("[dlsite-play] using cached worknos count={}", cached.len());
...
fn extract_work_type_filter(filters: &[FilterValue]) -> (Vec<String>, Vec<String>) {
	for f in filters {
		if let FilterValue::MultiSelect {
			...
			&& id == "work_type" && (!included.is_empty() || !excluded.is_empty()) {
...
fn extract_sort_filter(filters: &[FilterValue]) -> (SortOption, bool) {
	...
	(
		settings::get_default_sort(),
		settings::get_default_sort_ascending(),
	)
}
...
fn extract_content_rating_filter(filters: &[FilterValue]) -> Vec<String> {
	for f in filters {
		if let FilterValue::MultiSelect { id, included, .. } = f
			&& id == "content_rating" && !included.is_empty() {
				return included.clone();
			}
	}
	Vec::new()
...
mod tests {
	...
	fn play_viewer_url_simple() {
		...
		assert_eq!(
			url,
			"https://play.dlsite.com/work/RJ274802/view/images%2Fpage1.jpg"
		);
	}
